...
VulkanPipeline::VulkanPipeline(GLFWwindow* window) noexcept
	...
	for (uint32_t j = 0; j < extensionsCount; ++j)
		glfwExtensions.push_back(extensions[j]);

	gInstance = std::make_shared<Utils::Instance>(&debugReportCallbackFn, glfwExtensions);
...
VulkanPipeline::~VulkanPipeline() {
	...
	vkDestroySurfaceKHR(gInstance->getNativeHandle(), mSurface, nullptr);
